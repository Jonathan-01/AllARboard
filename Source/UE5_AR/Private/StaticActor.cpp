...
void AStaticActor::BeginPlay()
{
	...
	if (random == 0)
	{
		...
		BuildingType = TypeEnum::Red;
	}
	else if (random == 1)
	{
		// Add to GREEN class
		StaticMeshComponent->SetMaterial(0, GreenMaterial);
		BuildingType = TypeEnum::Green;
	}
	else if (random == 2)
	{
		// Add to BLUE class
		StaticMeshComponent->SetMaterial(0, BlueMaterial);
		BuildingType = TypeEnum::Blue;
	}
	//StaticMeshComponent->SetMaterial(0, MaterialInstance);
	GEngine->AddOnScreenDebugMessage(-1, 3.0f, FColor::Purple, TEXT("Colour Changed"));
}
